// src/features/jobs/SortableJobList.jsx
...
function Row({ job, handleProps, onEdit, onArchive }) {
  return (
    <div className="card" style={{ display:'flex', justifyContent:'space-between', alignItems:'center', gap:10 }}>
      <div style={{ display:'flex', alignItems:'center', gap:10 }}>
        <span
          ...
          style={{ cursor:'grab', padding:'4px 8px', border:'1px solid var(--border)', borderRadius:6, background:'#f8fafc' }}
        >
          ║║
        </span>
        <div>
      ...
      <div className="row">
        <button className="btn" onClick={(e)=>{ e.stopPropagation(); onEdit(job); }}>Edit</button>
        <button
          className="btn"
          style={{ borderColor: job.status==='active'?'#ef4444':'#16a34a', color: job.status==='active'?'#ef4444':'#16a34a' }}
...
function SortableItem({ job, onEdit, onArchive }) {
  ...
  const handleProps = { ...attributes, ...listeners }; // apply only to the small handle

  return (
    <div ref={setNodeRef} style={style}>
      <Row job={job} handleProps={handleProps} onEdit={onEdit} onArchive={onArchive} />
    </div>
  );
}
  ...
  return (
    ...
    >
      <SortableContext items={items.map(i=>i.order)} strategy={verticalListSortingStrategy}>
        <div className="list">
          {items.map(job => <SortableItem key={job.id} job={job} onEdit={onEdit} onArchive={onArchive} />)}
        </div>
      </SortableContext>
    </DndContext>
  );
